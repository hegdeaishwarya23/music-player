...
def favourites():
    qry = c.execute('SELECT * from fav')
    for i in qry:
        listboxfav.insert(END, i)
...
listboxfav = Listbox(root)
listboxfav.pack(fill=X)
...
def fav():
    sng = listbox.curselection()
    sng = int(sng[0])
    play_it = playlist[sng]
    fav2 = os.path.basename(play_it)
    print(fav2)
    print(type(fav2))
    c.execute("INSERT INTO fav VALUES(?);", (fav2,))
    conn.commit()
...
favbutton = Button(root, image=favphoto, width=25, height=20, command=fav)
favbutton.pack()
